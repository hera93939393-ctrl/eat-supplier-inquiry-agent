...
> **최종 결과**: 의도 분류 정확도 **93% (28/30, macro F1 0.933)** / 1턴 답변 통과율 **100% (30/30)**
> — 10라운드 실험 끝에 도달한 최종 수치다(전체 과정은 §4 참고). 아래 §4에 나오는 중간 라운드
...
**이 표는 §4 요약표의 2번째 행과 같은 중간 결과다 — 최종 수치는 93%/100%(맨 위 요약 박스,
§4 마지막 라운드).**
...
편향 수정)에서 ①이 87%→93%로 한 번 더 올랐다. 진짜 최종 수치는 리포트 맨 위 요약 박스 참고.**
...
87%는 아직 최종이 아니다 — §4 마지막 라운드에서 93%까지 더 올랐다.)
...
아키텍처 요구사항도 충족했다(`experiments/gpt4o_split_final.json`, `.log`). (①은 바로 다음
라운드에서 93%까지 한 번 더 올랐다.)
...
②도 100%로 복귀했다. **최종: ①93%(28/30, macro F1 0.933) / ②100%(30/30)**
(`experiments/gpt4o_final_best.json`, `.log`). 남은 오류 2건(#5, #24)은 두 카테고리 문서에
같은 사실이 정당하게 겹쳐 있는, 실사용에 영향 없는 경계 사례로 최종 확인됐다.
...
|---|---|---|---|---|---|
| **document_review** | 5 | 0 | 1 | 0 | 0 |
| **site_inspection** | 0 | 6 | 0 | 0 | 0 |
| **procedure_timeline** | 0 | 0 | 6 | 0 | 0 |
| **eligibility_restriction** | 0 | 1 | 0 | 5 | 0 |
| **out_of_scope** | 0 | 0 | 0 | 0 | 6 |

대각선 밖의 값은 딱 2개뿐이다 — document_review→procedure_timeline 1건(#5), eligibility_restriction
→site_inspection 1건(#24). 둘 다 앞서 확인했듯 "정답 카테고리 문서와 실제 배정된 카테고리 문서
양쪽에 같은 사실이 들어있어 답변 자체는 정확한" 경계 사례다. out_of_scope 열/행이 완전히 깨끗한
것도 확인된다 — 진짜 무관한 질문을 엉뚱한 주제로 착각하거나, 반대로 정상 질문을 범위 밖으로
잘못 넘긴 사례가 최종적으로 0건이라는 뜻이다.
...
(①93%/F1 0.933, ②100%까지 개선, §4 마지막 라운드). 시간에 쫓길 때보다 여유 있게 실패 사례를
하나씩 다시 읽을 때 이런 미묘한 편향이 더 잘 보였다.
...
2. 카테고리 간 사실 중복(예: "현장심사 수진자 제한"이 site_inspection·eligibility_restriction
   두 문서에 겹침)이 여전히 남아있다(#5, #24) — 지금은 같은 사실을 양쪽 문서에 중복 기재하는
